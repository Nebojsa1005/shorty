...
const authRoutes = (app) => {
    app.post("/api/auth/sign-up", async (req, res) => {
        try {
            ...
            return ServerResponse.serverError(res, 500, "Something Went Wrong", error);
        }
    });
    app.post("/api/auth/sign-in", async (req, res) => {
        ...
        if (!user) {
            return ServerResponse.serverError(res, 400, "There is No User with Entered Email");
        }
        const verifiedPassword = await compare(userData.password, user.password);
        if (!verifiedPassword)
    ...
    app.get("/api/auth/google-login", async (req, res) => {
        ...
        return ServerResponse.serverSuccess(res, 200, "Login Successful", userToSend);
    });
};
