...
const urlRoutes = (app) => {
    ...
    app.get("/api/url/get-by-short-link-id/:shortLinkId", async (req, res) => {
    ...
    app.post("/api/url/create", async (req, res) => {
        try {
            const { formData, userId } = req.body;
            if (!formData.destinationUrl) {
                return ServerResponse.serverError(res, 404, "Destination URL not found");
            }
    ...
    app.put("/api/url/edit/:id", async (req, res) => {
        ...
        try {
            const updatedUrlLink = await UrlModel.findByIdAndUpdate(id, {
                ...urlForm,
            }, {
                new: true,
            });
            return ServerResponse.serverSuccess(res, 200, "Successfully Updated Minified Link", updatedUrlLink);
        }
        catch (error) {
    ...
    app.post("/api/url/delete/:id", async (req, res) => {
        ...
        try {
            ...
            return ServerResponse.serverSuccess(res, 200, "Successfully Deleted Minified Url");
        }
        catch (error) {
            return ServerResponse.serverError(res, 404, error.message, error);
        }
    ...
    app.get("/api/url/password-check/:password/:id", async (req, res) => {
        ...
        try {
            ...
            if (!shortLink) {
                return ServerResponse.serverError(res, 404, "No Link With This Url Found");
            }
            console.log(shortLink);
            const verifiedPassword = await compare(password, shortLink.password);
            if (verifiedPassword) {
